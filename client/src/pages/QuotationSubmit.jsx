...
const QuotationSubmit = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Submit Quotation</h1>
        <p className="text-slate-500 dark:text-slate-400">RFQ: {rfq.title} — Deadline: {new Date(rfq.deadline).toLocaleDateString()}</p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-8 transition-colors">
        <form onSubmit={handleSubmit} className="space-y-8">
          <div>
            <h2 className="text-lg font-bold text-slate-800 dark:text-white border-b dark:border-slate-800 pb-3 mb-4">Line Items Pricing</h2>
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-800/50 text-sm text-slate-500 dark:text-slate-400 uppercase">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {items.map((item, index) => (
                    <tr key={index}>
                      <td className="p-3 font-medium text-slate-800 dark:text-slate-300">{item.name}</td>
                      <td className="p-3 text-slate-600 dark:text-slate-400">{item.quantity}</td>
                      <td className="p-3 text-slate-600 dark:text-slate-400">{item.unit}</td>
                      <td className="p-3">
                        <input 
                          type="number" 
                          required 
                          min="0"
                          className="w-full p-2 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 dark:text-white rounded-xl focus:ring-2 focus:ring-green-500/50 text-sm"
                  ...
                  <tr className="bg-slate-50 dark:bg-slate-800/50 font-bold text-slate-800 dark:text-white">
                    ...
                    <td className="p-4 text-right text-lg text-green-700 dark:text-green-400">₹{calculateTotal().toLocaleString('en-IN')}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div className="grid grid-cols-2 gap-6 pt-4 border-t border-slate-100 dark:border-slate-800">
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Delivery Time (Days) *</label>
              <input 
                type="number" 
                required 
                min="1"
                className="w-full p-2 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 dark:text-white rounded-xl focus:ring-2 focus:ring-green-500/50"
                value={deliveryDays}
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Terms / Notes</label>
              <textarea 
                rows="2" 
                className="w-full p-2 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 dark:text-white rounded-xl focus:ring-2 focus:ring-green-500/50"
                value={notes}
          ...
          <div className="flex justify-end pt-4 space-x-4">
            <button 
              ...
              className="px-8 py-2.5 rounded-xl font-medium text-slate-700 dark:text-slate-300 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors"
            >
              Save as Draft
            </button>
            <button 
