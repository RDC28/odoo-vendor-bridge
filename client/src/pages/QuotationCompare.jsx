  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center mb-6">
        <Link to="/rfqs" className="mr-4 text-slate-400 hover:text-slate-600"><ArrowLeft className="h-6 w-6" /></Link>
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Quotation Comparison</h1>
          <p className="text-slate-500">RFQ: {rfq.title} — {quotations.length} quotations received</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr>
                <th className="p-4 bg-slate-50 border-b border-r border-slate-200 w-1/4">Line Items</th>
                {quotations.map((quote, idx) => (
                  <th key={quote._id} className={`p-4 border-b border-slate-200 text-center ${quote.totalAmount === lowestTotal ? 'bg-green-50' : 'bg-slate-50'}`}>
                    <div className="font-bold text-slate-800 text-lg mb-1">{quote.vendor?.name || `Vendor ${idx+1}`}</div>
                    <div className="flex items-center justify-center mb-1 text-sm text-yellow-500">
                      <span>{'★'.repeat(Math.floor(4.5))}</span>
                      <span className="text-slate-400 ml-1 text-xs">4.5/5.0</span>
                    </div>
                    <div className="text-xs text-slate-500 font-normal">Delivery: {quote.deliveryDays} Days</div>
                    {quote.totalAmount === lowestTotal && (
                      <span className="inline-block mt-2 bg-green-100 text-green-800 text-xs px-2 py-1 rounded-full font-bold">Best Price</span>
                    )}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              ...
              {rfq.items.map((item, itemIdx) => (
                <tr key={itemIdx} className="border-b border-slate-100">
                  ...
                  {quotations.map(quote => {
                    ...
                    return (
                      <td key={quote._id} className={`p-4 text-center ${quote.totalAmount === lowestTotal ? 'bg-green-50/30' : ''}`}>
                        <div className="text-slate-800 font-medium">₹{quoteItem ? quoteItem.unitPrice.toLocaleString('en-IN') : 'N/A'}</div>
                        <div className="text-xs text-slate-500">Total: ₹{quoteItem ? quoteItem.totalPrice.toLocaleString('en-IN') : 'N/A'}</div>
                      </td>
                    );
                  })}
                </tr>
              ...
              <tr className="border-b-2 border-slate-200">
                <td className="p-4 border-r border-slate-200 font-bold text-right text-slate-800">Grand Total:</td>
                {quotations.map(quote => (
                  <td key={quote._id} className={`p-4 text-center text-xl font-bold ${quote.totalAmount === lowestTotal ? 'bg-green-50 text-green-700' : 'text-slate-800'}`}>
              ...
              {/* Notes Row */}
              <tr>
                <td className="p-4 border-r border-slate-200 font-medium text-slate-600 bg-slate-50/50">Vendor Notes:</td>
                {quotations.map(quote => (
                  <td key={quote._id} className={`p-4 text-sm text-slate-600 ${quote.totalAmount === lowestTotal ? 'bg-green-50/30' : ''}`}>
                    {quote.notes || '-'}
                  </td>
                ))}
              </tr>
              ...
              <tr>
                ...
                {quotations.map(quote => (
                  <td key={quote._id} className={`p-4 text-center ${quote.totalAmount === lowestTotal ? 'bg-green-50/30' : ''}`}>
                    {rfq.status !== 'Approved' && rfq.status !== 'Closed' && rfq.status !== 'Under Review' && (
                      <button 
