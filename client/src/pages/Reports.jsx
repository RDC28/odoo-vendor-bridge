  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-slate-500">Procurement spend analysis and vendor performance.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
          <h2 className="text-lg font-bold text-slate-800 mb-6">Monthly Spend Trend</h2>
          <div className="h-80">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
          <h2 className="text-lg font-bold text-slate-800 mb-6">Spend by Category</h2>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-lg font-bold text-slate-800">Top Vendors by Spend</h2>
          <button 
            ...
            <tbody className="divide-y divide-slate-100 text-sm">
              {topVendors.length === 0 ? (
                ...
                <tr key={vendor._id} className="hover:bg-slate-50">
                  ...
                  <td className="p-4 font-medium text-slate-900">{vendor.vendorName}</td>
                  <td className="p-4 text-slate-600">{vendor.category}</td>
                  <td className="p-4 text-right text-slate-600">{vendor.poCount}</td>
                  <td className="p-4 text-right font-medium text-slate-900">₹{(vendor.totalSpend || 0).toLocaleString('en-IN')}</td>
                </tr>
              ))}
            </tbody>
          </table>
