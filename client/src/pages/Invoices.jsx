  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-slate-500 dark:text-slate-400">Manage billing and payments.</p>
      </div>
              ...
              {loading ? (
                ...
                invoices.map((inv) => (
                  <tr key={inv._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    <td className="p-4">
                      <div className="font-medium text-slate-900 dark:text-white">{inv.invoiceNumber}</div>
                      <div className="text-slate-500 dark:text-slate-400 text-xs mt-1">Ref: {inv.purchaseOrder?.poNumber}</div>
                    </td>
                    ...
                    <td className="p-4 text-right flex justify-end items-center space-x-2">
                      <button onClick={() => setSelectedInvoice(inv)} className="text-slate-600 dark:text-slate-400 hover:text-green-600 dark:hover:text-green-400 hover:bg-green-50 dark:hover:bg-green-900/30 px-2 py-1.5 rounded-md transition-colors inline-flex items-center text-xs" title="View Details">
      ...
      {selectedInvoice && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 dark:bg-slate-900/80 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 dark:border dark:border-slate-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="flex justify-between items-center p-6 border-b border-slate-100 dark:border-slate-800">
              <div>
                <h2 className="text-lg font-bold text-slate-800 dark:text-white">Invoice Details</h2>
                <p className="text-sm text-slate-500 dark:text-slate-400">{selectedInvoice.invoiceNumber}</p>
              </div>
              <button onClick={() => setSelectedInvoice(null)} className="text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300">
            ...
            <div className="p-6">
              <div className="grid grid-cols-2 gap-6 mb-6">
                <div>
                  ...
                  <p className="font-medium text-slate-800 dark:text-white">{selectedInvoice.vendor?.name}</p>
                </div>
                <div>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">Date Created</p>
                  <p className="font-medium text-slate-800 dark:text-white">{format(new Date(selectedInvoice.createdAt), 'MMM dd, yyyy')}</p>
                </div>
                <div>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">PO Reference</p>
                  <p className="font-medium text-slate-800 dark:text-white">{selectedInvoice.purchaseOrder?.poNumber || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">Due Date</p>
                  <p className="font-medium text-slate-800 dark:text-white">{format(new Date(selectedInvoice.dueDate), 'MMM dd, yyyy')}</p>
                </div>
              </div>

              <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-5 border border-slate-100 dark:border-slate-800">
                <h3 className="font-bold text-slate-800 dark:text-white mb-4">Amount Breakdown</h3>
                <div className="space-y-3">
                  ...
                  <div className="pt-3 border-t border-slate-200 dark:border-slate-700 flex justify-between font-bold text-slate-800 dark:text-white text-lg">
                    <span>Grand Total:</span>
                    <span className="text-green-600 dark:text-green-400">₹{(selectedInvoice.totalAmount || 0).toLocaleString('en-IN')}</span>
                  </div>
                </div>
              </div>
            </div>
            <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 flex justify-end space-x-3">
              ...
              <button 
                ...
                className="px-4 py-2 bg-slate-200 text-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700 hover:bg-slate-300 rounded-xl font-medium transition-colors"
              >
                Close
              </button>
            </div>
